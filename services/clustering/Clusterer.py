...
class Clusterer:
    ...
    def add_document(self, document_id):
        ...
        cluster_similarites = [(idx, self.similarity(vector, centroid)) for idx, centroid in enumerate(self.store.get_centroids())]
...
class MemoryClusterStore:
    def __init__(self, centroids, documents, threshold):
        if len(centroids) != len(documents): raise RuntimeError("centoids and documents must have same length")
    ...
    def set_centroid(self, cluster_id, centroid):
        if cluster_id >= len(self.documents): raise RuntimeError("Invalid cluster id")
